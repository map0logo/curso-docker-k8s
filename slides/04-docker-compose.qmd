...
## Problemas con aplicaciones multi-contenedor
...
- Una sola comando para levantar toda la aplicación
...
## Services: El corazón de Compose 2
...
## Redes: ¿Cómo se comunican los contenedores? 2

```{mermaid}
graph LR
  A[Frontend] -->|"http:#47;#47;api:3000"| B(API)
...
## Dependencias y orden de inicio de los servicios 2
...
## Variables de entorno y configuraciones 2
...
```{mermaid}
graph TB
  A[".env file"] --> B[Interpolación de Compose]
  C["env_file:"] --> B
  B --> D[COnfiguración Final]
...
## Volúmenes y persistencia en Compose 2
...
```{mermaid}
graph TB
    A[Named Volume] --> B[Persiste entre despliegues]
...
## Docker Compose vs Docker Swarm 2
...
## Comportamiento de los Health Checks
...
## Patrones comunes de orquestación 2
...
## Buenas prácticas con Docker Compose 2
...
```{mermaid}
graph TD
  ComposeFile["docker-compose.yml"] --> ServicioAPI["
  api:
    build: ./fastapi
      ports: [8000:8000]
    environment:
