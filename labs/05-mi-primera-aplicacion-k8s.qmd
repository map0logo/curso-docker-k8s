...
```powershell
systeminfo | Select-String "Hyper-V|VMX|SVM"
```
...
Esto es opcional en caso de no tener instalado Docker, y docker no es una opción viable en tu maquina.
...
- Descargar [`minikube-installer.exe`](https://storage.googleapis.com/minikube/releases/latest/minikube-installer.exe) desde GitHub Releases
...
Minikube incluye una versión integrada de kubectl, pero es recomendable instalarlo por separado ya que la versión de minikube es limitada:
...
Prueba si puedes acceder a la aplicación desde tu navegador.
...
# Verificar que todas las réplicas estén ejecutándose
...
Guarda manualmente el valor de `SERVICE_URL`

```bash
SERVICE_URL=http://127.0.0.1:35091 # Cambia esto con el resultado de la primera solicitud
...
minikube start --driver=docker --nodes=2 --cpus=2 --memory=4g --disk-size=20g --profile=mi-cluster
...
**Ejercicio 1:** Crea un nuevo deployment con una aplicación simple que muestre "Hola Mundo" y hazlo escalar a 5 réplicas.
